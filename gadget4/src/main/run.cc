...
void sim::run( void )
{
    ...
    void write_potential_tracers( char filename[], double potentials[], double coordinates[][ 3 ],
                                  int ids[], double& time,
                                  int& num );  // the function to write potential tracers to hdf5
                                               // file, only called by the root rank
    void collect_potential_tracers( double localPot[], double localPos[][ 3 ], int localIDs[],
                                    double globalPot[], double globalPos[][ 3 ], int globalIDs[],
                                    int& localNum, int& globalNum, int& rank, int& size );
    MyReal( *initPos )[ 3 ];  // backup of the initial coordinates of the potential tracer particles
                              // in global process aim: avoid the numerical error of the coordinates
                              // correction which is multiple summation of the position shift (a
                              // small number) in the double precision
    initPos = new MyReal[ Sp.TotNumPart ][ 3 ];  // copy of the init coordinates of the potential
                                                 // tracers, release by OS when the program ends
    int firstIDofPotTracer =
        INT_MAX;  // the starting ID of the potential tracer particles in global process
    ...
    MyIntPosType intpos[ 3 ] = { 0, 0, 0 };  // unsigned integer coordinates
    double       centerOfMass[ 3 ] = { 0.0, 0.0, 0.0 };  // center of mass
    double       offset = 0.0;  // offset for the particle coordinates w.r.t. the center of mass
    // initialize the center of mass
    double comNumerator[ 3 ] = { 0.0, 0.0, 0.0 };  // local sum of coordinates
    double comDenominator    = 0.0;                // local sum of Mass
    double oldValue[ 3 ]     = { centerOfMass[ 0 ], centerOfMass[ 1 ],
                                 centerOfMass[ 2 ] };  // old center of mass
#endif
    ...
    while ( 1 ) /* main loop over synchronization points */
    {
        ...
        Sp.drift_all_particles();  // we do this here to be able to avoid large buffer sizes, if
                                   // needed multiple binning operations are done
#endif
#endif
        ...
        /* compute SPH densities and smoothing lengths for active SPH particles, and optionally
         * those accessed passively. This also creates the list of active hydro particles at this
        ...
        /* if particles have increased their smoothing lengths, this is recorded in parent tree
         * nodes */
        NgbTree.update_maxhsml();

        /* hydro-forces, second half-step. This will also update the predicted velocities/entropies
         * with the new current ones */
        do_hydro_step_second_half();

        /* this does the closing gravity half-step for the timebins that end at the current
        ...
        if ( All.Ti_Current >= TIMEBASE
             || All.Time > All.TimeMax ) /* did we reached the final time? */
        {
        ...
        /* Find new hydro timesteps. This will not change the set of active hydro particles at this
         * synchronization point, but it can change how they are distributed over timebins. */
        find_hydro_timesteps();

        /* compute hydro-forces and apply momentum changes to interacting particle pairs for first
         * half-steps */
        ...
        /* output some CPU usage log-info (accounts for everything needed up to complete the
         * previous timestep) */
        Logs.write_cpu_log();
        ...
        {
            mpi_printf( "RUN: We have reached the timestep specified with STOP_AFTER_STEP and "
                        "therefore stop." );
            endrun();
        }
#endif
        ...
        for ( int i = 0; i < Sp.NumPart; ++i )  // collect the data of the potential tracers
        {
            if ( Sp.P[ i ].getType() == All.PotTracerType )
            {
                ...
                idPotTracer[ numPotTracer++ ] =
                    i;  // get the number and id of the potential tracers
            }
            else if ( Sp.P[ i ].getType() == All.RecenterPartType )
                idRecenter[ numRecenter++ ] =
                    i;  // get the number and id of the recentering anchors
        }
        ...
        // output the coordinates and potentials of the potential tracers at specified output steps
        {
            ...
            if ( ThisTask == 0 )  // only the root rank writes the data to the file
                write_potential_tracers( All.PotOutFile, potGlobal, posGlobal, pIDsGlobal, All.Time,
                                         numPotTracerTot );

            if ( All.NumCurrentTiStep == 0 )  // backup the position of the potential tracers at the
                                              // beginning of the simulation
            {
                memset( initPos, 0,
                        Sp.TotNumPart * 3 * sizeof( MyReal ) );  // initialize the array to 0
                if ( numPotTracer > 0 )
                    // only calculate the minimum if there are potential tracers, to avoid garbage
                    // value
                    firstIDofPotTracer = *( std::min_element( partIDs, partIDs + numPotTracer ) );
                else
                    firstIDofPotTracer = INT_MAX;  // set the first ID to a large value if there is
                                                   // no potential tracer, so that the following
                                                   // MPI_Allreduce will not change the value
                MPI_Allreduce( MPI_IN_PLACE, &firstIDofPotTracer, 1, MPI_INT, MPI_MIN,
                               Communicator );  // get the first ID of the potential tracer
                                                // particles in global process
        ...
        {
            double factor =
                All.NumCurrentTiStep == 0 ? 100.0 : 1;  // the scale factor for region size: set a
                                                        // large region for the 1st iteration
            ...
            for ( int i = 0; i < numRecenter;
                  ++i )  // calculate the center of mass: denominator and numerator in local process
            {
                Sp.intpos_to_pos( Sp.P[ idRecenter[ i ] ].IntPos, pos );
                // only consider the particles within the specified radius
                offset =
                    sqrt( ( pos[ 0 ] - centerOfMass[ 0 ] ) * ( pos[ 0 ] - centerOfMass[ 0 ] )
                          + ( pos[ 1 ] - centerOfMass[ 1 ] ) * ( pos[ 1 ] - centerOfMass[ 1 ] )
                          + ( pos[ 2 ] - centerOfMass[ 2 ] ) * ( pos[ 2 ] - centerOfMass[ 2 ] ) );
                if ( offset
                     < All.RecenterSize
                           * factor )  // only consider the particles within the specified radius
                {
        ...
        {
            MyReal pos[ 3 ] = {
                initPos[ partIDs[ i ] - firstIDofPotTracer ][ 0 ] + centerOfMass[ 0 ],
        ...
        double        coordinates[ Sp.NumPart ][ 3 ];  // coordinates array
        static double pos[ 3 ];
#endif  // use the coordinates and potentials of the potential tracer particles to save memory if
        // ZERO_MASS_POT_TRACER is defined
        int    ids[ Sp.NumPart ];
        ...
        // API of galotfa
        galotfa_without_pot_tracer( ids, types, masses, coordinates, velocities, All.Time,
                                    Sp.NumPart );
#endif
    ...
    // output the positions and potentials of the potential tracers at specified output steps
    {
                                          ...
                                          // beginning of the simulation
        {
            ...
            if ( numPotTracer > 0 )
                ...
                firstIDofPotTracer =
                    INT_MAX;  // set the first ID to a large value if there is no potential tracer,
                              // so that the following MPI_Allreduce will not change the value
            MPI_Allreduce( MPI_IN_PLACE, &firstIDofPotTracer, 1, MPI_INT, MPI_MIN,
                           Communicator );  // get the first ID of the potential tracer particles in
                                            // global process
            for ( int i = 0; i < numPotTracer; ++i )
            {
                initPos[ partIDs[ i ] - firstIDofPotTracer ][ 0 ] =
    ...
    for ( int loop = 0; loop < 25; ++loop )  // MAX number of iterations = 25
    {
        double factor =
            All.NumCurrentTiStep == 0
                ? 100.0
                : 1;  // the scale factor for region size: set a large region for the 1st iteration
        memset( comNumerator, 0, 3 * sizeof( double ) );
        comDenominator = 0.0;
        // backup the old value
        oldValue[ 0 ] = centerOfMass[ 0 ];
    ...
    // API of galotfa
    galotfa_without_pot_tracer( ids, types, masses, coordinates, velocities, All.Time, Sp.NumPart );
#endif
    restart Restart{ Communicator };
    Restart.write( this ); /* write a restart file at final time - can be used to continue
                              simulation beyond final time */
 ...
 * particles at this time, i.e. the particles at the sync-point have finished their regular
 * timestep.
 *
 */
...
int sim::check_for_interruption_of_run( void )
{
    ...
    if ( ThisTask == 0 )
    {
        ...
        if ( ( fd = fopen( stopfname,
                           "r" ) ) ) /* Is the stop-file present? If yes, interrupt the run. */
        {
            fclose( fd );
        ...
        if ( ( fd = fopen( stopfname, "r" ) ) ) /* Is the restart-file present? If yes, write a
                                                   user-requested restart file. */
        {
            fclose( fd );
        ...
        if ( Logs.CPUThisRun > 0.85 * All.TimeLimitCPU ) /* are we running out of CPU-time ? If yes,
                                                            interrupt run. */
        {
    ...
    if ( All.OutputListOn )
    {
        for ( int i = 0; i < All.OutputListLength; i++ )
        {
            ...
            if ( time >= All.TimeBegin && time <= All.TimeMax )
            {
                ...
                /* We will now modify 'ti' to map it to the closest available output time according
                 * to the specified MaxSizeTimestep. The real output time may hence deviate by  +/-
                 * 0.5*MaxSizeTimestep from the desired output time.
                 */
...
#ifndef OUTPUT_NON_SYNCHRONIZED_ALLOWED
            /* We will now modify 'ti' to map it to the closest available output time according to
             * the specified MaxSizeTimestep. The real output time may hence deviate by  +/-
             * 0.5*MaxSizeTimestep from the desired output time.
             */
...
void sim::create_snapshot_if_desired( void )
{
    ...
    /* we may do this on partial timesteps since for massmaps we always drift all particles, i.e.
     * the lightcone is complete up to All.Time */
    LightCone.lightcone_massmap_flush( 1 );
#endif

#ifndef OUTPUT_NON_SYNCHRONIZED_ALLOWED
    if ( All.HighestActiveTimeBin
         == All.HighestOccupiedTimeBin ) /* allow only top-level synchronization points */
#endif
        if ( All.Ti_Current >= All.Ti_nextoutput && All.Ti_nextoutput >= 0 )
        {
            ...
            // do an extra domain decomposition here to make sure that there are no new stars among
            // the block of gas particles
            NgbTree.treefree();
            Domain.domain_free();
            Domain.domain_decomposition( STANDARD );
            NgbTree.treeallocate( Sp.NumGas, &Sp, &Domain );
            ...
            /* this structure will hold auxiliary information for each particle, needed only during
             * group finding */
            Sp.PS = ( subfind_data* )Mem.mymalloc_movable( &Sp.PS, "PS",
                                                           Sp.MaxPart * sizeof( subfind_data ) );
            memset( Sp.PS, 0, Sp.MaxPart * sizeof( subfind_data ) );

            /* First, we save the original location of the particles, in order to be able to revert
             * to this layout later on */
            for ( int i = 0; i < Sp.NumPart; i++ )
            {
            ...
            for ( int n = 0; n < Sp.NumPart; n++ )
            {
                ...
                if ( Sp.P[ n ].PrevSubhaloNr.get() >= MergerTree.PrevTotNsubhalos
                     && Sp.P[ n ].PrevSubhaloNr.get() != HALONR_MAX )
                ...
                if ( Sp.P[ n ].PrevSizeOfSubhalo.get() > 0
                     && Sp.P[ n ].PrevSubhaloNr.get() == HALONR_MAX )
...
#ifdef SUBFIND_ORPHAN_TREATMENT
            {
                ...
                Snap.write_snapshot(
                    ...
                    MOST_BOUND_PARTICLE_SNAPHOT ); /* write special snapshot file */
            }
#endif
            ...
            /* we need to reconstruct the timebins here. Even though the particles are in the same
             * place again, it could have happened that Sp.P was reduced in size temporarily below
             * NumPart on a certain task, in which case the timebin data may have become invalid.
             */
    ...
    if ( Lp.TestIfAboveFillFactor( std::min< int >( Lp.MaxPart, Sp.MaxPart ) ) )
    {
        ...
        {
            mpi_printf( "\nLIGHTCONE_PARTICLES_GROUPS: We shall first compute a group catalogue "
                        "for the lightcone particles\n" );
            ...
            /* this structure will hold auxiliary information for each particle, needed only during
             * group finding */
            Lp.PS = ( subfind_data* )Mem.mymalloc_movable( &Lp.PS, "PS",
                                                           Lp.MaxPart * sizeof( subfind_data ) );
...
#ifdef MERGERTREE
                MergerTree.Ntrees = 0;
                lightcone_particle_io Lcone( &Lp, &LightCone, &MergerTree, Communicator,
                                             All.SnapFormat ); /* get an I/O object */
#else
            lightcone_particle_io Lcone( &Lp, &LightCone, Communicator,
                                         All.SnapFormat ); /* get an I/O object */
#endif
                long long NumLP_tot = Lp.NumPart;
                MPI_Allreduce( MPI_IN_PLACE, &NumLP_tot, 1, MPI_LONG_LONG, MPI_SUM, Communicator );
                mpi_printf( "\nLIGHTCONE: writing particle lightcone conesnap files #%d ... "
                            "(NumLP_tot = %lld)\n",
                            All.LightconeFileCount, NumLP_tot );
                ...
                /* let's put this in brackets so that the object's destructor will be called already
                 * here */
            }
            ...
            mpi_printf( "LIGHTCONE_PARTICLES_GROUPS: We could store %lld particles from the "
                        "buffer, but had to keep %lld\n",
                        nsum[ 0 ], nsum[ 1 ] );
        }
#else
        Lp.NumPart = 0;
        ...
        if ( Lp.MaxPart > LIGHTCONE_ALLOC_FAC * Sp.MaxPart + 1
             && Lp.NumPart < LIGHTCONE_ALLOC_FAC * Sp.MaxPart )
                                              ...
                                              // file, only called by the root rank
{
    ...
    static hid_t file_id =
        H5Fcreate( potFile, H5F_ACC_TRUNC, H5P_DEFAULT, H5P_DEFAULT );  // open the file
    ...
    hid_t dataset_coord = H5Dcreate2( group_id, "Coordinates", H5T_NATIVE_DOUBLE, dataspace_id_2d,
                                      H5P_DEFAULT, prop_id_2d, H5P_DEFAULT );
    hid_t dataset_pot   = H5Dcreate2( group_id, "Potentials", H5T_NATIVE_DOUBLE, dataspace_id_1d,
                                      H5P_DEFAULT, prop_id_1d, H5P_DEFAULT );
    hid_t dataset_id    = H5Dcreate2( group_id, "TracerIDs", H5T_NATIVE_INT, dataspace_id_1d,
                                      H5P_DEFAULT, prop_id_1d, H5P_DEFAULT );
    ...
    hid_t attr_space =
        H5Acreate2( group_id, "Time", H5T_NATIVE_DOUBLE, attr_id, H5P_DEFAULT, H5P_DEFAULT );
    ...
    if ( size > 1 )  // if there are more than one MPI tasks
    {
        if ( rank == 0 )
        {
            ...
            for ( int i = 1; i < size; ++i )
            {
                ...
                MPI_Recv( globalPot + offset, dataTransferSize, MPI_DOUBLE, i, i, MPI_COMM_WORLD,
                          MPI_STATUS_IGNORE );
                MPI_Recv( globalPos + offset, 3 * dataTransferSize, MPI_DOUBLE, i, 1e5 + i,
                          MPI_COMM_WORLD, MPI_STATUS_IGNORE );
                MPI_Recv( globalIDs + offset, dataTransferSize, MPI_INT, i, 2e5 + i, MPI_COMM_WORLD,
                          MPI_STATUS_IGNORE );
        ...
        else
        {
            for ( int i = 1; i < size; ++i )
                if ( rank == i )
                {
                    MPI_Send( &localNum, 1, MPI_INT, 0, 0,
                              MPI_COMM_WORLD );  // send the size of data to be sent
