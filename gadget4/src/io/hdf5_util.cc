...
static herr_t hdf5_conv_half( hid_t src, hid_t dst, H5T_cdata_t* cdata, size_t nelmts,
                              size_t buf_str, size_t bkg_str, void* buf, void* background,
                              hid_t plist )
{
...
void my_create_HDF5_halfprec_handler( void )
{
    ...
    H5Tregister( H5T_PERS_SOFT, "half-converter", Halfprec_memtype, Halfprec_memtype,
                 hdf5_conv_half );
}
 ...
 */
hid_t my_H5Dcreate( hid_t loc_id, const char* datasetname, hid_t type_id, hid_t space_id,
                    hid_t dcpl_id )
{
    hid_t dataset_id =
        H5Dcreate( loc_id, datasetname, type_id, space_id, H5P_DEFAULT, dcpl_id, H5P_DEFAULT );
 ...
 */
herr_t my_H5Dwrite( hid_t dataset_id, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
                    hid_t xfer_plist_id, const void* buf, const char* datasetname )
{
    herr_t status =
 ...
 */
hid_t my_H5Acreate( hid_t loc_id, const char* attr_name, hid_t type_id, hid_t space_id,
                    hid_t acpl_id )
{
 ...
 */
herr_t my_H5Dread( hid_t dataset_id, hid_t mem_type_id, hid_t mem_space_id, hid_t file_space_id,
                   hid_t xfer_plist_id, void* buf, const char* datasetname )
{
    herr_t status =
 ...
 */
herr_t my_H5Aread( hid_t attr_id, hid_t mem_type_id, void* buf, const char* attr_name,
                   hssize_t size )
{
    ...
    if ( attr_size != size )
    {
        ...
        H5Eset_auto( H5E_DEFAULT, NULL, NULL );
        Terminate( "Error detected in HDF5: mismatch in size for attribute %s, expected size = "
                   "%lld, actual attribute size = %lld\n",
                   attr_name, ( long long int )size, ( long long int )attr_size );
    }
 ...
 */
herr_t my_H5Sselect_hyperslab( hid_t space_id, H5S_seloper_t op, const hsize_t* start,
...
int read_scalar_attribute( hid_t handle, const char* attr_name, const char* alternative_name,
...
void read_vector_attribute( hid_t handle, const char* attr_name, void* buf, hid_t mem_type_id,
                            int length )
{
    hid_t hdf5_attribute = my_H5Aopen_name( handle, attr_name );
...
void write_scalar_attribute( hid_t handle, const char* attr_name, const void* buf,
                             hid_t mem_type_id )
{
    if ( H5Aexists( handle, attr_name ) )
...
void write_vector_attribute( hid_t handle, const char* attr_name, const void* buf,
                             hid_t mem_type_id, int length )
{
    if ( H5Aexists( handle, attr_name ) )
        H5Adelete( handle, attr_name );
